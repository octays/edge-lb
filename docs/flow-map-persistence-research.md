...
当前只按 listener socket、协议、VIP、target 业务地址和端口匹配当前 endpoint，
然后重映射当前 `listener_id` / `target_id`。地址不匹配则计入
`skipped{reason="config"}`，不写入 eBPF。

旧版本按 `target_id` 将对端 overlay 地址映射成本地 overlay 的 fallback 已删除。
统一业务地址后两台 gateway 不需要这种转换，也不能仅凭相同下标把旧会话恢复到
不同业务地址。详情见 [DNAT 业务地址修复](dnat-service-address-fix.md)。
